...
`static/swagger-ui/` is also **gitignored** (generated by `make swagger-ui-copy` from `node_modules/swagger-ui-dist`). The `//go:embed swagger-ui` pattern in `static/static.go` fails on a fresh checkout with `pattern swagger-ui: no matching files found`. The Dockerfile and every CI before_script (`make swagger-ui-copy`) materialize it; mirror that locally with `make swagger-ui-copy` before any `go vet`/`go test`/`go build` on a fresh clone.
...
- `static/swagger-ui/*` is built by `make swagger-ui-copy` from `node_modules/swagger-ui-dist` and is **gitignored**. Never hand-edit those files.
