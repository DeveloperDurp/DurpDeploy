...
func TestAgentLabelPages_HideWritesForViewerAndRenderResponsiveMembers(t *testing.T) {
	// Given
	viewer := auth.SetUser(httptest.NewRequest("GET", "/admin/agent-labels/1", nil),
		&db.User{Role: "viewer"})
	...
	// When
	markup := renderAgentAdminPage(t, viewer.Context(), AgentLabelDetailPage(view))
