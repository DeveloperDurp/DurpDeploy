...
WHERE audit_log.created_at < ?
  AND NOT EXISTS (SELECT 1 FROM deployments WHERE audit_log.entity_type = 'deployment' AND deployments.id = audit_log.entity_id)
  AND NOT EXISTS (SELECT 1 FROM releases WHERE audit_log.entity_type = 'release' AND releases.id = audit_log.entity_id)
