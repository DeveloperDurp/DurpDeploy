...
case "$DB_KIND" in
    sqlite)
        DB_DSN="${DURPDEPLOY_DB:-durpdeploy.db}"
        DB_CLIENT="sqlite"
        ;;
    postgres|postgresql)
        DB_KIND=postgres
    ...
    *)
        ...
        echo "       DURPDEPLOY_DB_KIND=postgres DURPDEPLOY_DB=... $0"
        exit 2
        ;;
esac
...
if [[ -n "$DB_CONTAINER" ]]; then
    ...
    case "$DB_CLIENT" in
        sqlite) command -v sqlite3 >/dev/null || { echo "FAIL: sqlite3 is required" >&2; exit 1; } ;;
        postgres) command -v psql >/dev/null || { echo "FAIL: psql is required" >&2; exit 1; } ;;
        mssql) command -v sqlcmd >/dev/null || { echo "FAIL: sqlcmd is required" >&2; exit 1; } ;;
    esac
fi
...
db_query() {
    local query=$1
    case "$DB_CLIENT" in
        sqlite) sqlite3 "$DB_DSN" "$query" ;;
        postgres)
            if [[ -n "$DB_CONTAINER" ]]; then
                docker exec "$DB_CONTAINER" psql -U durpdeploy -d durpdeploy -At -c "$query"
            else
                psql "$DB_DSN" -At -c "$query"
            fi
            ;;
        mssql)
            if [[ -n "$DB_CONTAINER" ]]; then
...
# sqlcmd may pad text columns; normalize the value before sending it back as
# a form token so the application sees the exact token stored in the session.
CSRF=$(db_query "SELECT LTRIM(RTRIM(csrf_token)) FROM sessions WHERE id='$SESSION_ID';")
...
LOG_LINES=$(db_query "SELECT line FROM deployment_logs WHERE deployment_id=$RETRY_DEP ORDER BY id;")
...
APP_PROD_STAGE_ID=$(db_query "SELECT id FROM lifecycle_stages WHERE lifecycle_id=$APP_LC_ID AND environment_id=$APP_PROD_ID;")
...
db_query "SELECT approved_by FROM deployment_approvals WHERE deployment_id=$PROD_DEP;" | grep -q "$ADMIN_EMAIL" || { echo "FAIL: approval not recorded"; exit 1; }
...
ADMIN_CSRF=$(db_query "SELECT csrf_token FROM sessions WHERE id='$ADMIN_SESSION_ID';")

# Promote the new user to admin via PUT /admin/users/{id}.
NEW_USER_ID=$(db_query "SELECT id FROM users WHERE email='$NEW_EMAIL';")
CODE=$(curl -s -b "$COOKIES" -o /dev/null -w "%{http_code}" \
...
NEW_ROLE=$(db_query "SELECT role FROM users WHERE id=$NEW_USER_ID;")
...
SESSION_EXISTS=$(db_query "SELECT COUNT(*) FROM sessions WHERE id='$NEW_SESSION_ID' AND user_id=$NEW_USER_ID;")
...
USER_GONE=$(db_query "SELECT COUNT(*) FROM users WHERE id=$NEW_USER_ID;")
...
USER_AUDIT=$(db_query "SELECT COUNT(*) FROM audit_log WHERE action IN ('create_user','update_user','delete_user');")
