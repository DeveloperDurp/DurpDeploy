...
`static/swagger-ui/` is also **gitignored** (generated by `make swagger-ui-copy` from `node_modules/swagger-ui-dist`). The `//go:embed swagger-ui` pattern in `static/static.go` fails on a fresh checkout with `pattern swagger-ui: no matching files found`. CI runs `make swagger-ui-copy` in `lint`/`test`/`build` before_script; mirror that locally with `make swagger-ui-copy` before any `go vet`/`go test`/`go build` on a fresh clone.
